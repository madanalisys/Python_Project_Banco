from python_project_3.models.cliente import Cliente
from python_project_3.utils.helper import formata_float_str_moeda
...
class Conta:
    ...
    def transferir(self: object, destino: object, valor: float) -> None:
        if 0 < valor <= self.saldo_total:   # if valor > 0 and self.saldo_total >= valor:
            if self.saldo >= valor:
                self.saldo = self.saldo - valor
                self.saldo_total = self._calcula_saldo_total
